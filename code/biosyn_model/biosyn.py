...
class BioSyn(object):
    ...
    #初始化sparse embedding的权重
    def init_sparse_weight(self, initial_sparse_weight, use_cuda):
        """
        Parameters
        ----------
        initial_sparse_weight : float 用来决定sparse和dense encoding的权重比
    ...
    def train_sparse_encoder(self, corpus):
        self.sparse_encoder = SparseEncoder().fit(corpus)#一个tf-idf，根据现有的语料库执行tf-idf的计算编码
    ...
    def embed_sparse(self, names, show_progress=False):
        ...
        Returns
        -------
        sparse_embeds : np.array 返回的也是array的形式，用concat来处理list of array
    ...
    def embed_dense(self, names, show_progress=False):
        ...
        with torch.no_grad():
            ...
            for start in iterations:
                ...
                batch_dense_embeds = batch_dense_embeds.cpu().detach().numpy()#detach?
